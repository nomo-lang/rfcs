...
| [0029](./rfcs/0029-bounded-utc-cron-schedule-calculation.md) | 受限 UTC Cron Schedule 计算 | Proposed（已提案） | cron、scheduling、time、Agent、bounds、browser WASM | 解析受限五字段 UTC schedule，并在不引入 process-global scheduler 的情况下确定性计算匹配分钟。 |
