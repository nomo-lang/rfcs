...
> **状态**：Implementation baseline（实现基线，发布稳定化中）
...
- `match` 必须穷尽所有变体。
- v0.1 暂不支持 `_` 通配分支。
- 核心 prelude 允许非限定 `Some`/`None`/`Ok`/`Err`；词法作用域中的同名变量或函数优先。用户自定义枚举仍要求 `Enum.Variant`，限定核心变体继续作为兼容和显式消歧写法。见已接受的 [RFC 0007](./rfcs/0007-unqualified-variant-access.md)。
...
每个 type parameter 当前最多声明一个 interface bound；调用必须提供显式 concrete
type argument。编译器验证泛型函数体只使用 interface 提供的操作，并验证 concrete
non-generic struct 存在匹配的 `impl Interface for Type`，然后单态化函数并静态分派
interface method。

当前 interface MVP 不支持多个 bound、`where`、type argument 推断、trait object、
associated type、blanket impl、动态分派、高阶类型或泛型特化。

该静态抽象边界由已接受的 [RFC 0010](./rfcs/0010-constrained-generics-and-static-interface-dispatch.md) 固化。
...
已接受的类型化 FFI 在不开放 raw pointer 的前提下扩展了这个边界：
...
opaque handle declaration 具有 nominal identity，不能在 Nomo 中构造，也不能与另一类
handle 混用。`Owned<T>` 与 `Borrowed<T>` 是受检查的 FFI ownership metadata；
`.borrow()` 创建 borrowed view，声明的 release function 必须消费匹配的 owned handle。
当前 preview 尚不执行完整 linear move 语义或自动析构。`Nullable<T>` 仅允许包装
handle，提供 `is_null()` 与显式 checked `unwrap()`，不会把 null 隐式转换成 handle。

callback parameter 只接受签名精确匹配、非捕获的 top-level function，并且只能使用
ABI-safe 类型。callback value 不能存储、返回或以其他方式逃逸；当前不支持 retained
callback、捕获/context trampoline 或从 foreign thread 进入。panic 采用 fail-fast，
不会跨 C frame unwind。`#[repr(C)]` struct 必须是 non-generic fixed-layout record，
field offset、size 与 alignment 按选定 target ABI 计算；extern 边界会拒绝不支持的 field
和未标记 `repr(C)` 的 struct。
...
`Opaque` 与 nominal handle 都不开放任意裸指针操作。核心 CLI 可以从受控 C header
子集生成可审查的 binding 与 provenance：
...
generator 支持 opaque struct typedef、固定字段 struct typedef、普通 function
declaration 和受限 function-pointer parameter；会拒绝 union、bitfield、array、
flexible array、variadic、多级 pointer 与未知 scalar spelling。pointer ownership 通过
有文档的确定性 release-name heuristic 推断，因此生成源码仍需 review。生成器不会执行
隐式 build-time code。

FFI 的调用点安全、所有权类型和 package linker metadata 边界由已接受的
[RFC 0011](./rfcs/0011-c-ffi-safety-and-link-boundary.md) 固化。
类型化 handle、nullability、受限 callback、C layout 与 binding generation 边界由
已接受的 [RFC 0019](./rfcs/0019-typed-ffi-handles-callbacks-and-bindings.md) 固化。
...
### 3.4 `Option` / `Result` 的编译器内建身份

`Option` 和 `Result` 既是标准库公共类型，又是编译器识别的核心 carrier。v0.1 由编译器提供其泛型枚举形状、核心 prelude 变体、`?` 规则与 C 后端布局；`std.option` / `std.result` 保持稳定的用户侧模块与 helper API。当前实现不依赖 `#[lang]` 属性或先行编译的 Nomo 标准库源码。见已接受的 [RFC 0006](./rfcs/0006-option-result-lang-items.md)。
...
- `Array.iter` 返回可被 `for ... in` 消费的快照值；通用 `Iterator` 抽象尚未进入当前标准库 API。

ARC/COW 的当前运行时策略由已接受的 [RFC 0003](./rfcs/0003-arc-cow-runtime-cost.md) 固化。
...
当前调用点检查强度由已接受的 [RFC 0004](./rfcs/0004-mutable-borrow-uniqueness.md) 固化。
...
- 没有显式 endpoint 的 registry/version source 在 v0.1 作为 lockfile 叶子节点记录。
  ...
  `metadata.json`。dependency manifest 可使用 exact version、caret range、tilde range
  或有界 comparison range；wildcard、alternative、隐式 `latest` 与 `=` exact 写法会被
  拒绝。fresh resolution 在整个项目或 workspace 的约束下确定性选择最高的非 yanked
  version，lockfile 只记录最终 exact version。HTTP package index 会缓存供 offline range
  resolution 使用。`--locked` 不重新求解，只校验锁定版本仍满足 manifest；
  `nomo deps update --precise` 也必须落在 manifest requirement 内。
  `nomo publish --dry-run` 会校验本地 package 并准备确定性的
...
- `nomo build [path] --target <triple>` 接受 canonical
  `arch-vendor-os-env` target 或标准三段式 Apple Darwin alias。显式 target 的
  工件隔离在 `build/<canonical-target>/{c,bin}`。target-aware C emission 会定义
  `std.os` 使用的 canonical triple、architecture 与 platform。首条 native
  cross-link 路径是 macOS `aarch64 <-> x86_64`；其它已识别的非 host target
  当前可使用 `--emit-c`，native linking 则会在缺少具体 toolchain 时失败。
...
当前包身份、图/lockfile 与 registry 协议分别由已接受的
[RFC 0008](./rfcs/0008-canonical-package-identity-and-aliases.md)、
[RFC 0009](./rfcs/0009-reproducible-workspace-and-package-graphs.md) 和
[RFC 0013](./rfcs/0013-registry-protocol-and-package-integrity.md) 固化。交互式 OAuth、
token refresh 和复杂版本求解留给后续；v0.1 遇到同一 canonical package id 的多版本
冲突直接报错。
...
- [RFC 0006](./rfcs/0006-option-result-lang-items.md)：`Option`/`Result` 编译器内建身份与标准模块契约。
- [RFC 0007](./rfcs/0007-unqualified-variant-access.md)：非限定枚举变体。
- [RFC 0008](./rfcs/0008-canonical-package-identity-and-aliases.md)：canonical 包身份与 dependency alias 分离。
- [RFC 0009](./rfcs/0009-reproducible-workspace-and-package-graphs.md)：可复现 workspace/package/module 图与 lockfile。
- [RFC 0010](./rfcs/0010-constrained-generics-and-static-interface-dispatch.md)：受约束泛型与 interface 静态分派。
