...
- `nomo doc [path] [--workspace] [--package <package>] [--std] [--open] [--json] [--output <dir>]`
  ...
  位置。struct field 与 enum variant 会作为子文档项输出，并进入 search index。默认写入
  `build/doc/index.html`、package/module HTML 页面与 `search-index.json`；`--json` 只输出
  机器可读文档模型，不写文件。`--workspace` 生成 workspace members 文档，`--package`
