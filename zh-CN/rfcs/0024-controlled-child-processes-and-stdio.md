...
| 创建日期 | 2026-07-24 |
| 实现状态 | 尚未接受；`std.process` 当前只有阻塞式 shell-command helper |
...
10. Formatting、Clippy、unit/CLI integration test、Linux/macOS real cross-build
    与 Windows native compile/run test 全部通过。
11. 实现通过签名提交、子分支、PR review、required CI 与绿色 post-merge main run
    合入；之后 RFC 才能转为 `Accepted`。
