...
Nomo v0.1 提供阻塞式、校验证书的 HTTPS client，并支持结构化请求、自定义
...
在本 RFC 落地前，Nomo 原生 CLI 可以打开 socket，并发出基础 `http://`
GET/POST，但无法安全调用模型 endpoint。OpenAI-compatible API 至少需要校验
HTTPS、`Authorization`
...
已接受实现于 2026-07-24 通过
[nomo PR #11](https://github.com/nomo-lang/nomo/pull/11) 合入。

| 表面 | 已接受证据 | 剩余缺口 |
| --- | --- | --- |
| 公共 API | `std/src/http.nomo` 定义 `HttpHeader`、`HttpRequest`、`HttpResponse`、`HttpError` 与 `send`，同时保留 `get` 和 `post` | Streaming 与 cancellation 留到 P1 |
| Compiler | `builtins_http.rs` 通过受控 intrinsic 降低结构化 request，并校验源码化标准库身份 | Reusable client state 与异步分派继续推迟 |
| C99 runtime | Unix-like target 动态加载 libcurl，Windows 动态加载 WinHTTP；两者都执行 TLS 校验、deadline、header/body cap 与稳定 error | Proxy 与 connection-pool policy 继续推迟 |
| 测试 | 本地 plain HTTP 与生成证书的 TLS fixture 覆盖成功、上限、TLS failure、稳定 code、secret redaction 与 Windows 原生执行 | Streaming/SSE 测试留到 P1 |
| 示例 | `examples/openai_compatible` 无需真实 key，通过本地 TLS fixture 发送非流式 chat-completions request | 结构化 response 解析依赖 P1 JSON 切片 |
| 规格 | 中英文 v0.1 规格暴露已接受的结构化 API 与 host-runtime contract | Browser 网络能力需要独立 host-capability 设计 |
...
在 Unix-like target 上，本地 TLS integration fixture 可以设置
`NOMO_HTTP_CA_BUNDLE` 指向临时 CA 文件。这是测试和受控开发 hook，并不是关闭
证书校验：该 CA 会成为 trust root，host-name verification 仍然必须执行。
Windows 使用 current-user 与 machine certificate store。生产默认使用平台 trust。
...
- Windows target：生成的 runtime 通过平台 loader 动态加载 WinHTTP；应用不写
  `winhttp`，也不增加 linker metadata。
...
Browser WASM interpreter 继续保持沙盒，并在求值或记录 request secret 前返回
稳定的 `NOMO-WASM-003` 网络 capability error。由浏览器提供 `fetch` capability
需要单独的 host-capability 设计，不属于原生 CLI acceptance gate。
...
| 方案 | 优点 | 缺点 | 倾向 |
| --- | --- | --- | --- |
| 工具链托管 libcurl/WinHTTP adapter | 保持 C99 产物；TLS/HTTP parser 成熟；应用无 FFI | 需要 runtime packaging 与平台 adapter 工作 | P0 已接受 |
...
7. Formatting、Clippy、unit test、CLI integration test、browser-WASM unsupported
   behavior、macOS arm64-to-x86_64 cross-build 与 Linux x86_64-to-arm64
   cross-build 已在
   [完整 CI run 30090585590](https://github.com/nomo-lang/nomo/actions/runs/30090585590)
   通过；Windows native compile/run path 已在
   [PR smoke run 30090447278](https://github.com/nomo-lang/nomo/actions/runs/30090447278)
   通过。
8. 实现通过签名提交、子分支与 PR #11 合入；合并后的
