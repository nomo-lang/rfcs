...
| 作者 | Nomo 语言工作组 |
| 创建日期 | 2026-06-18 |
| 实现状态 | 已落地：`match` 禁用 `_`；`let else` 与 `if let` 已覆盖 parser、类型检查、C 后端与示例测试 |
...
当前模式匹配规格以「强制穷尽性 + 让诊断覆盖所有缺失分支」为原则，继续禁用 `_` 通配分支；嵌套 `Option`/`Result` 的样板通过 `let else`、`if let` 和既有后缀 `?` 缓解。本 RFC 接受 A + C 组合，且该组合已经在 parser、类型检查、C 后端和示例测试中落地。
...
- **诊断**：新增名称解析/类型检查侧的 warning，例如 `E0410`（`_` 掩盖了可枚举的剩余变体）。
- **缺点**：放开后 AI/用户会倾向「一律 `_`」，逐步侵蚀穷尽性文化。

### 4.3 方案 C：引入 `if let` / `let else`（已接受）
...
| C `if let`/`let else`（已接受） | 单分支提取 + 发散 else | 保住 `match` 穷尽性、消除主要嵌套源 | 新增语法表面积 |
...
- **已在 v0.1 落地**：方案 C 的发散式 `let else` 与 `if let`；方案 D 由既有后缀 `expr?` 承担，用于返回 `Option` 的函数。
- **保持不变**：`match` 继续禁用 `_`（方案 A），并由 parser 与穷尽性诊断测试锁定。
- **推迟**：方案 B（受限 `_`）留作 v0.2 讨论，避免在 v0.1 削弱穷尽性文化。
...
接受 **A + C 组合**：`match` 保持禁用 `_`（守穷尽性），同时提供发散式 `let else` 与 `if let` 来消化嵌套 `Option`/`Result` 的主要样板来源。`else` 不发散会被类型检查拒绝；两种语法均已进入当前实现与验收测试。
...
- 后续若增加更丰富的嵌套解构，应继续复用当前穷尽性与绑定作用域规则。
